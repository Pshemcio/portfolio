...
export const TextReveal = styled.span`
    ...
    transition: transform 2s ${(props) =>
        typeof props.delay === 'number' ? props.delay.toString() + 's' : '.5s'} cubic-bezier(0.12, 1, 0.2, 1), opacity 2s ${(props) =>
            typeof props.delay === 'number' ? (props.delay + 0.2).toString() + 's' : '.7s'} cubic-bezier(0.12, .4, 0.2, 1);
    line-height: inherit;
